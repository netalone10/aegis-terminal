...
**Verdict:** Bullish bias. Golden cross confirmed on weekly. Enter on pullback to 9,650-9,720 zone with SL below 9,400. Target 10,200 (R:R 2.2:1).`,
  },
  {
    id: 3,
    role: 'user',
    content: 'What about the banking sector overall? Any rotation signals?',
  },
]
  ...
  return (
    <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : ''}`}>
      <div className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
        isUser ? 'bg-info/20' : 'bg-primary-bg'
      }`}>
      ...
      <div className={`max-w-[75%] rounded-lg px-4 py-3 ${
        isUser
          ...
          : 'bg-surface-dark border border-border-subtle'
      }`}>
        <div className="flex items-center gap-2 mb-1.5">
          <span className={`text-xs font-medium ${isUser ? 'text-info' : 'text-primary'}`}>
            {isUser ? 'You' : 'Aegis AI'}
          </span>
        ...
        <div className="text-sm text-fg-secondary leading-relaxed whitespace-pre-wrap">
          {msg.content.split('\n').map((line, i) => {
            if (line.startsWith('**') && line.endsWith('**')) {
              return <p key={i} className="font-semibold text-fg mt-2 mb-1">{line.replace(/\*\*/g, '')}</p>
            }
            if (line.startsWith('•')) {
              return <p key={i} className="pl-2 mt-0.5">{line}</p>
            }
...
export default function AI() {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="flex items-center gap-2 px-4 py-3 border-b border-border bg-default">
        <Sparkles size={14} className="text-primary" />
        <span className="text-sm font-medium text-fg">AI Assistant</span>
        <span className="text-xs text-fg-muted font-mono ml-1">· powered by Aegis</span>
      ...
      {/* Input bar */}
      <div className="border-t border-border bg-default p-4">
        <div className="flex items-center gap-3">
          <input
            ...
            className="flex-1 px-4 py-2.5 bg-surface border border-border rounded-lg text-sm text-fg placeholder:text-fg-placeholder focus:outline-none focus:border-primary transition-colors"
          />
          <button
            className="p-2.5 bg-primary text-canvas rounded-lg hover:bg-primary-hover transition-colors disabled:opacity-40"
            disabled={!input.trim()}
          >
            <Send size={16} />
          </button>
        </div>
        <p className="text-[10px] text-fg-placeholder mt-2 text-center">
